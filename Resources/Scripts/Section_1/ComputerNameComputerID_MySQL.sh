...
for computerID in $computerIDs; do
    computerName=$(mysql --defaults-file=/tmp/my.cnf -jamf.pro.server.here database_name_here -BsN -e " SELECT computer_name FROM computers_denormalized WHERE computer_id = '$computerID' ")
